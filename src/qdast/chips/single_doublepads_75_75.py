...
class SingleDoublepads7575(QDASTChip):
    ...
    def _produce_qubits(self):
        qubit_spacing_x = 2200  # x-distance between qubits on the same feedline side
        qubit_spacing_x_alt = 600 # x-distance between qubits on different feedline sides
...
####################
    ...
    def _produce_chargelines(self):
        ...
        cross_trans_0 = pya.CplxTrans(
            1, 0, False, pya.DPoint(self.launchers["S"][0].x, self.launchers["S"][0].y + 400 + self.b + self.a/2)
        )
        cross_trans_1 = pya.CplxTrans(
            1, 180, False, pya.DPoint(self.launchers["N"][0].x, self.launchers["N"][0].y - 400 + self.b + self.a/2)
        )
        ...
        self.insert_cell(
            taper_cell, pya.CplxTrans(1, 90, False, pya.DPoint(self._qubit_refpoints[0]["port_drive"].x, tee_refpoints_0["port_left"].y + 400))
        )
        self.insert_cell(
            taper_cell, pya.CplxTrans(1, 90 + 180, False, pya.DPoint(self._qubit_refpoints[1]["port_drive"].x, tee_refpoints_1["port_right"].y - 400))
        )
        ...
        self.insert_cell(
            taper_cell, pya.CplxTrans(1, 90, False, pya.DPoint(self._qubit_refpoints[2]["port_drive"].x, tee_refpoints_0["port_left"].y + 400))
        )
        self.insert_cell(
            taper_cell, pya.CplxTrans(1, 90 + 180, False, pya.DPoint(self._qubit_refpoints[3]["port_drive"].x, tee_refpoints_1["port_right"].y - 400))
        )
        self._produce_waveguide(
            [
    ...
    def _produce_feedline(self, x_distance):
        ...
        wg0 = self._produce_waveguide(
            [
                self.launchers["W"][0],
                pya.DPoint(
                    self.launchers["W"][0].x + x_distance,
                    self.launchers["W"][0].y,
                ),
                tee_refpoints[0]["port_left"],
            ]
        )
        ...
        wg4 = self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self.launchers["E"][0].x - x_distance,
                    self.launchers["E"][0].y,
                ),
                self.launchers["E"][0],
            ]
        )
