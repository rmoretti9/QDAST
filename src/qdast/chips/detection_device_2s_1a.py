...
class DetectionDevice2s1a(QDASTChip):
    sensing_1_coupler_widths = Param(pdt.TypeList, "S1 coupler widths", [100, 100])
    sensing_2_coupler_widths = Param(pdt.TypeList, "S2 coupler widths", [110, 110])
    ancilla_coupler_widths = Param(pdt.TypeList, "A1 coupler widths", [105, 105, 105])
    ...
    def build(self):
        ...
        self._produce_feedline_resonator(
            mirror_x=1, mirror_y=1, id=0, l1_id="9", l2_id="10"
        )
        self._produce_feedline_resonator(
            mirror_x=-1, mirror_y=1, id=1, l1_id="7", l2_id="6"
        )
        self._produce_feedline_resonator(
            mirror_x=1, mirror_y=-1, id=2, l1_id="1", l2_id="12"
        )
    ...
    def _produce_qubits(self):
        ...
        qubit = self.add_element(
            ...
            coupler_widths=[
                float(self.sensing_1_coupler_widths[0]),
                0,
                0,
                float(self.sensing_1_coupler_widths[1]),
                0,
                0,
            ],
            ...
            width_untapered=6,
            drive_position=[
                self._driveline_terminations[0].x - 3500,
                self._driveline_terminations[0].y - 4000,
            ],
        ...
        qubit = self.add_element(
            ...
            island_extent=[540, 240],
            coupler_widths=[
                float(self.sensing_2_coupler_widths[0]),
                0,
                0,
                float(self.sensing_2_coupler_widths[1]),
                0,
                0,
            ],
            island_to_island_distance=50,
            ...
            width_untapered=6,
            drive_position=[
                self._driveline_terminations[1].x - 6500,
                self._driveline_terminations[1].y - 4000,
            ],
            external_leads_offset=305,
            bent_section_length=6,
            lead_height_tapered=4,
        ...
        qubit = self.add_element(
            ...
            coupler_widths=[
                ...
                0,
                0,
                float(self.ancilla_coupler_widths[0]),
                float(self.ancilla_coupler_widths[1]),
                float(self.ancilla_coupler_widths[2]),
            ],
            island_to_island_distance=50,
            ...
            width_untapered=6,
            drive_position=[
                self._driveline_terminations[2].x - 5000,
                self._driveline_terminations[2].y - 7000,
            ],
            external_leads_offset=305,
            bent_section_length=6,
            lead_height_tapered=4,
    ...
    def _produce_feedline_resonator(self, mirror_x, mirror_y, id, l1_id, l2_id):
        ...
        feedline_tee_trans = pya.DCplxTrans(
            ...
            rot_tee,
            False,
            self.launchers[l2_id][0].x + mirror_x * 250,
        ...
        w3 = self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    ...
                    + mirror_x * (-150 - (self.a / 2 + self.b) * sqrt(2) / 2),
                    self.launchers[l2_id][0].y
                    + mirror_y * (-500 + (self.a / 2 + self.b) * sqrt(2) / 2),
                ),
            ]
        )
        if mirror_x == 1 and mirror_y == 1:
        ...
        resonator_tee, resonator_tee_refp = self.insert_cell(
            cell_cross, resonator_tee_trans
        )
        ...
        cplr_dtrans = pya.DCplxTrans(
            ...
            False,
            self.launchers[l1_id][0].x,
            self.launchers[l1_id][0].y + mirror_y * 800,
        )
        ...
        cplr_res_dtrans = pya.DCplxTrans(
            1,
            rot_cplr,
            False,
            pya.DPoint(
                resonator_tee_refp["port_bottom"].x
                + mirror_x * cplr_length * sqrt(2) / 2,
                resonator_tee_refp["port_bottom"].y
                + mirror_y * cplr_length * sqrt(2) / 2,
            ),
        )
        ...
        self._readout_structure_info["feedline_" + str(id)] = (
            w5.length(),
            w4.length(),
            w3.length(),
            meander_1_length,
    ...
    def _produce_readout_reasonators(self):
        ...
        wg2 = self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self._cplr_res_refpoints[2]["port_b"].x + 100,
                    self._cplr_res_refpoints[2]["port_b"].y - 100,
                ),
                pya.DPoint(
                    self._cplr_res_refpoints[2]["port_b"].x + 100,
                    self._qubits_refpoints[2]["port_5"].y,
                ),
                pya.DPoint(
                    self._cplr_res_refpoints[2]["port_b"].x + 200,
                    self._qubits_refpoints[2]["port_5"].y,
                ),
            ]
        )
        wg2_2 = self._produce_waveguide(
            [
                pya.DPoint(
                    self._cplr_res_refpoints[2]["port_b"].x + 1800,
                    self._qubits_refpoints[2]["port_5"].y,
                ),
                pya.DPoint(
                    self._qubits_refpoints[2]["port_5"].x,
                    self._qubits_refpoints[2]["port_5"].y,
                ),
            ]
        )
        ...
        w = 1000
        meander_length = (
            float(self.readout_res_lengths[2]) - wg2.length() - wg2_2.length()
        )
        num_meanders = _get_num_meanders(meander_length, 50, w)
        self.insert_cell(
            Meander,
    ...
    def _produce_drivelines(self):
        ...
        self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self.launchers["11"][0].x + 1500, self.launchers["11"][0].y - 600
                ),
                pya.DPoint(
                    self.launchers["11"][0].x + 2000, self.launchers["11"][0].y - 600
                ),
            ]
        )
        ...
        _, taper_ref1 = self.insert_cell(taper_cell, taper_trans_1)
        self._produce_waveguide(
            [taper_ref1["port_b"], taper_ref1["port_b"] + pya.DPoint(200, 0)],
            a=self.a / 2,
            b=self.b / 2,
            term2=self.b,
        )
        ...
        self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self.launchers["4"][0].x - 300, self.launchers["4"][0].y - 400
                ),
                pya.DPoint(
                    self.launchers["4"][0].x - 1850, self.launchers["4"][0].y - 1900
                ),
                pya.DPoint(
                    self.launchers["4"][0].x - 1850, self.launchers["4"][0].y - 2600
                ),
                pya.DPoint(
                    self.launchers["4"][0].x - 2150, self.launchers["4"][0].y - 2900
                ),
                # pya.DPoint(self.launchers['4'][0].x - 2100, self.launchers['4'][0].y - 200),
            ]
        )
        taper_trans_2 = pya.DCplxTrans(
            1,
            -135,
            False,
            pya.DPoint(
        ...
        _, taper_ref2 = self.insert_cell(taper_cell, taper_trans_2)
        self._produce_waveguide(
            [
                taper_ref2["port_b"],
        ...
        taper_trans_3 = pya.DCplxTrans(
            1,
            -90,
            False,
            pya.DPoint(self.launchers["2"][0].x, self.launchers["2"][0].y - 1400),
        )
        _, taper_ref3 = self.insert_cell(taper_cell, taper_trans_3)
        self._produce_waveguide(
            [
                taper_ref3["port_b"],
        ...
        self._driveline_terminations.append(
            pya.DPoint(taper_ref1["port_b"].x + 200, taper_ref1["port_b"].y)
        )
        self._driveline_terminations.append(
            pya.DPoint(taper_ref2["port_b"].x - 100, taper_ref2["port_b"].y - 100)
        )
    ...
    def _produce_fluxlines(self):
        self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self.launchers["8"][0].x - 400,
                    self._qubits_refpoints[0]["port_flux"].y,
                ),
        ...
        self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self.launchers["5"][0].x - 300,
                    self._qubits_refpoints[1]["port_flux"].y,
                ),
        ...
        self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self.launchers["3"][0].x - 400,
                    self._qubits_refpoints[2]["port_flux"].y,
                ),
    ...
    def _produce_couplers(self):
        ...
        wg1_2 = self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self._qubits_refpoints[0]["port_0"].x + 800,
                    self._qubits_refpoints[2]["port_4"].y,
                ),
        ...
        w = 1000
        meander_length = (
            float(self.coupler_res_length[0]) - wg1.length() - wg1_2.length()
        )
        num_meanders = _get_num_meanders(meander_length, 50, w)
        self.insert_cell(
            Meander,
        ...
        wg2_2 = self._produce_waveguide(
            [
                ...
                pya.DPoint(
                    self._qubits_refpoints[2]["port_3"].x,
                    self._qubits_refpoints[1]["port_0"].y + 1900,
                ),
        ...
        w = 1000
        meander_length = (
            float(self.coupler_res_length[1]) - wg2.length() - wg2_2.length()
        )
        num_meanders = _get_num_meanders(meander_length, 50, w)
        self.insert_cell(
            Meander,
