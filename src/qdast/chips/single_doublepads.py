...
class SingleDoublepads(QDASTChip):
    ...
    def build(self):
        ...
        """
        self.launchers = self.produce_launchers("6-ports-10x10", launcher_assignments={})
        ...
        _, self.cl_2 = self.insert_cell(launcher_cell, pya.DCplxTrans(1, 90, False, 453 + 200, 10000 - 620 - 200))
        _, self.cl_3 = self.insert_cell(launcher_cell, pya.DCplxTrans(1, 90, False, 10000 - 453 - 200, 10000 - 615 - 200))
        _, self.refpoints_fl_out = self.insert_cell(launcher_cell, pya.DCplxTrans(1, 0, False, 10000 - 620 - 200, 5000))
        _, self.cl_1 = self.insert_cell(launcher_cell, pya.DCplxTrans(1, -90, False, 10000 - 453 - 200, + 615 + 200))
        _, self.cl_0 = self.insert_cell(launcher_cell, pya.DCplxTrans(1, -90, False, 453 + 200 + 7, + 615 + 200))
    ...
    def _produce_waveguide(self, path, term2=0, turn_radius=None, a=None, b=None, object=None):
        ...
        waveguide = self.add_element(
            WaveguideCoplanar, path=pya.DPath(path, 1), r=turn_radius, term2=term2, a=a, b=b
        )
    ...
    def _produce_qubits(self):
        ...
        qb0_refpoints = self._produce_qubit(float(self.coupler_widths[0]), qubits_center_x - qubit_spacing_x, y_b, 0, "qb_0")
        qb1_refpoints = self._produce_qubit(float(self.coupler_widths[1]), qubits_center_x - qubit_spacing_x + qubit_spacing_x_alt, y_a, 2, "qb_1")
        qb2_refpoints = self._produce_qubit(float(self.coupler_widths[2]), qubits_center_x, y_b, 0, "qb_2")
        qb3_refpoints = self._produce_qubit(float(self.coupler_widths[3]), qubits_center_x + qubit_spacing_x_alt, y_a, 2, "qb_3")
        qb4_refpoints = self._produce_qubit(float(self.coupler_widths[0]), qubits_center_x + qubit_spacing_x, y_b, 0, "qb_4")
        qb5_refpoints = self._produce_qubit(float(self.coupler_widths[1]), qubits_center_x + qubit_spacing_x + qubit_spacing_x_alt, y_a, 2, "qb_5")
        ...
        self._qubit_refpoints = [qb0_refpoints, qb1_refpoints, qb2_refpoints, qb3_refpoints, qb4_refpoints, qb5_refpoints]
    ...
    def _produce_readout_resonator(self, capacitor, capacitor_dtrans, res_idx, qubit_refp):
        ...
        cross_trans1 = pya.CplxTrans(1, -90, False, pya.DPoint(qubit_refp["port_0"].x, capacitor_pos.y - flip * (150 + self.a / 2 + self.b)))
        ...
        wg_1 = self._produce_waveguide([capacitor_pos, pya.DPoint(capacitor_pos.x, tee_refpoints[tee_port_1].y)])
        wg_2 = self._produce_waveguide([tee_refpoints[tee_port_2], pya.DPoint(qubit_refp["port_0"].x, qubit_refp["port_0"].y + flip * 300), pya.DPoint(qubit_refp["port_0"].x, qubit_refp["port_0"].y)])
        wg_3 = self._produce_waveguide([tee_refpoints["port_bottom"], pya.DPoint(tee_refpoints["port_bottom"].x - 300, tee_refpoints["port_bottom"].y), pya.DPoint(tee_refpoints["port_bottom"].x - 500, tee_refpoints["port_bottom"].y), pya.DPoint(tee_refpoints["port_bottom"].x - 500, tee_refpoints["port_bottom"].y - flip * 100)], turn_radius=turn_radius)
    ...
    def _produce_feedline_resonator(self):
        ...
        for i in range(6):
            cross_trans = pya.DTrans(tee_rotations[i], False, self._qubit_refpoints[i]["port_0"].x, 5e3)
        ...
        cplr_params = cap_params(self.feedline_capacitor_n_fingers, float(self.l_fingers[0]), "smooth", finger_gap=self.b)
        ...
        self._produce_waveguide([self.refpoints_fl_in["base"], cplr_refpoints["port_a"]], object="feedline")
        ...
        self._produce_waveguide([tee_refpoints[3]["port_left"], tee_refpoints[4]["port_left"]], object="feedline")
        self._produce_waveguide([tee_refpoints[4]["port_right"], tee_refpoints[5]["port_right"]], object="feedline")
        self._produce_waveguide([tee_refpoints[5]["port_left"], feedline_tee_refp["port_left"]], object="feedline")
        self._produce_waveguide([feedline_tee_refp["port_right"], self.refpoints_fl_out["base"]], object="feedline")
        self._produce_waveguide([feedline_tee_refp["port_bottom"], pya.DPoint(feedline_tee_refp["port_bottom"].x, feedline_tee_refp["port_bottom"].y - 500), pya.DPoint(feedline_tee_refp["port_bottom"].x + 200, feedline_tee_refp["port_bottom"].y - 500), pya.DPoint(feedline_tee_refp["port_bottom"].x + 200, feedline_tee_refp["port_bottom"].y - 1200)], object="feedline")

    def _produce_readout_resonators(self):
        """Create readout resonators for each coupler/tee pair."""
        for i in range(6):
            cplr_params = cap_params(float(self.n_fingers[i]), float(self.l_fingers[i]), self.type_coupler[i], finger_gap=self.b)
            cplr = self.add_element(**cplr_params)
            cplr_refpoints_rel = self.get_refpoints(cplr)
            if i % 2 == 0:
                cplr_pos = self._tee_refpoints[i]["port_bottom"] - pya.DTrans.R90 * cplr_refpoints_rel["port_b"]
            else:
                cplr_pos = self._tee_refpoints[i]["port_bottom"] + pya.DTrans.R90 * cplr_refpoints_rel["port_b"]
    ...
    def _produce_chargelines(self):
        ...
        cross_length = self.b * 3 + 2 * self.a
        cross_trans_0 = pya.CplxTrans(1, 90, False, pya.DPoint(self._qubit_refpoints[0]["port_drive"].x, self.cl_0["base"].y + 1000 + self.b + self.a / 2))
        cross_trans_1 = pya.CplxTrans(1, -90, False, pya.DPoint(self._qubit_refpoints[5]["port_drive"].x, self.cl_2["base"].y - 1000 - self.b - self.a / 2))
        _, tee_refpoints_0 = self.insert_cell(cell_cross, cross_trans_0)
        _, tee_refpoints_1 = self.insert_cell(cell_cross, cross_trans_1)

        taper_cell, _ = WaveguideCoplanarTaper.create_with_refpoints(self.layout, self.LIBRARY_NAME, a=self.a, b=self.b, a2=self.a / 3, b2=self.b / 3, taper_length=80)
        _, taper_ref0 = self.insert_cell(taper_cell, pya.CplxTrans(1, 90, False, pya.DPoint(tee_refpoints_0["port_right"].x, tee_refpoints_0["port_right"].y + 1000)))
        _, taper_ref1 = self.insert_cell(taper_cell, pya.CplxTrans(1, -90, False, pya.DPoint(self._qubit_refpoints[1]["port_drive"].x, tee_refpoints_1["port_right"].y - 1000)))
        _, taper_ref2 = self.insert_cell(taper_cell, pya.CplxTrans(1, 90, False, pya.DPoint(self._qubit_refpoints[2]["port_drive"].x, tee_refpoints_0["port_right"].y + 1000)))
        _, taper_ref3 = self.insert_cell(taper_cell, pya.CplxTrans(1, -90, False, pya.DPoint(self._qubit_refpoints[3]["port_drive"].x, tee_refpoints_1["port_right"].y - 1000)))
        _, taper_ref4 = self.insert_cell(taper_cell, pya.CplxTrans(1, 90, False, pya.DPoint(self._qubit_refpoints[4]["port_drive"].x, tee_refpoints_0["port_right"].y + 1000)))
        _, taper_ref5 = self.insert_cell(taper_cell, pya.CplxTrans(1, -90, False, pya.DPoint(self._qubit_refpoints[5]["port_drive"].x, tee_refpoints_1["port_right"].y - 1000)))

        # Tapered lines connecting tapers to qubit drive ports
        self._produce_waveguide([taper_ref0["port_b"], self._qubit_refpoints[0]["port_drive"]], a=self.a / 3, b=self.b / 3, term2=self.b)
        self._produce_waveguide([taper_ref1["port_b"], self._qubit_refpoints[1]["port_drive"]], a=self.a / 3, b=self.b / 3, term2=self.b)
        self._produce_waveguide([taper_ref2["port_b"], self._qubit_refpoints[2]["port_drive"]], a=self.a / 3, b=self.b / 3, term2=self.b)
        self._produce_waveguide([taper_ref3["port_b"], self._qubit_refpoints[3]["port_drive"]], a=self.a / 3, b=self.b / 3, term2=self.b)
        self._produce_waveguide([taper_ref4["port_b"], self._qubit_refpoints[4]["port_drive"]], a=self.a / 3, b=self.b / 3, term2=self.b)
        self._produce_waveguide([taper_ref5["port_b"], self._qubit_refpoints[5]["port_drive"]], a=self.a / 3, b=self.b / 3, term2=self.b)

        # BOTTOM ROW WIRING
        self._produce_waveguide([self.cl_0["base"], pya.DPoint(self.cl_0["base"].x, self.cl_0["base"].y + 200), pya.DPoint(self._qubit_refpoints[0]["port_drive"].x, self.cl_0["base"].y + 200), pya.DPoint(self._qubit_refpoints[0]["port_drive"].x, self.cl_0["base"].y + 300)])
        self._produce_waveguide([pya.DPoint(self._qubit_refpoints[0]["port_drive"].x, self.cl_0["base"].y + 300), tee_refpoints_0["port_left"]])
        self._produce_waveguide([tee_refpoints_0["port_right"], pya.DPoint(tee_refpoints_0["port_right"].x, tee_refpoints_0["port_right"].y + 1000)])

        self._produce_waveguide([tee_refpoints_0["port_bottom"], pya.DPoint(self._qubit_refpoints[2]["port_drive"].x, tee_refpoints_0["port_bottom"].y), taper_ref2["port_a"]])
        self._produce_waveguide([self.cl_1["base"], pya.DPoint(self.cl_1["base"].x, self.cl_1["base"].y + 200), pya.DPoint(self._qubit_refpoints[4]["port_drive"].x, self.cl_1["base"].y + 200), pya.DPoint(self._qubit_refpoints[4]["port_drive"].x, taper_ref4["port_a"].y)])

        # TOP ROW WIRING
        self._produce_waveguide([self.cl_2["base"], pya.DPoint(self.cl_2["base"].x, self.cl_2["base"].y - 200), pya.DPoint(self._qubit_refpoints[1]["port_drive"].x, self.cl_2["base"].y - 200), pya.DPoint(self._qubit_refpoints[1]["port_drive"].x, taper_ref1["port_a"].y)])
        self._produce_waveguide([self.cl_3["base"], pya.DPoint(self.cl_3["base"].x, self.cl_3["base"].y - 200), pya.DPoint(self._qubit_refpoints[5]["port_drive"].x, self.cl_3["base"].y - 200), pya.DPoint(self._qubit_refpoints[5]["port_drive"].x, self.cl_3["base"].y - 300)])
        self._produce_waveguide([tee_refpoints_1["port_right"], pya.DPoint(tee_refpoints_1["port_right"].x, tee_refpoints_1["port_right"].y - 1000)])
        self._produce_waveguide([tee_refpoints_1["port_bottom"], pya.DPoint(self._qubit_refpoints[3]["port_drive"].x, tee_refpoints_1["port_bottom"].y), pya.DPoint(self._qubit_refpoints[3]["port_drive"].x, taper_ref3["port_a"].y)])
        self._produce_waveguide([pya.DPoint(self._qubit_refpoints[5]["port_drive"].x, self.cl_3["base"].y - 300), tee_refpoints_1["port_left"]])
