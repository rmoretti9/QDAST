...
from qdast.junctions.manhattan_single_junction_centered import ManhattanSingleJunctionCentered
...
class Clockmon(Qubit):
    ...
    coupler_offset = Param(pdt.TypeDouble, "Distance between coupler ad qubit origin", 200, unit="μm")
    ...
    island_to_island_distance = Param(pdt.TypeDouble, "Island to island distance", 50, unit = "μm")
    ...
    def build(self):
        ...
        etch_region = ground_gap_region - island_region + coupler_gap_region - coupler_region

        # SQUID
        squid_transf = pya.DCplxTrans(1, -self.bending_angle, False, pya.DVector(0, self.squid_offset))
        if self.with_squid:
            self.produce_squid(squid_transf, include_base_metal_gap = False, include_base_metal_addition = False)
        else:
    ...
    def _build_coupler(self):
        ...
        coupler_region = pya.Region(pya.DPolygon(coupler_points).to_itype(self.layout.dbu)).round_corners(5 / self.layout.dbu, 5 / self.layout.dbu, self.n)
        waveguide_region = pya.Region(pya.DPolygon(waveguide_points).to_itype(self.layout.dbu))
        total_coupler_region = coupler_region + waveguide_region
        
        if has_gap:
            total_coupler_gap_region = total_coupler_region.sized(self.b / self.layout.dbu, self.b / self.layout.dbu, self.n)
            etch_top_waveguide_region = pya.Region(pya.DBox(-self.a/2 - self.b, offset + height + stem_height, self.a/2+ self.b, offset + height + stem_height + self.b).to_itype(self.layout.dbu))
            total_coupler_gap_region -= etch_top_waveguide_region
            protection_region = total_coupler_gap_region.sized(self.margin/self.layout.dbu)
            self.add_protection(protection_region)
    ...
    def _build_island(self):
        ...
        island_region = pya.Region(pya.DPolygon([
        ...
        if self.clock_diameter > self.island_to_island_distance:
            circular_unecth = pya.Region(pya.DPolygon(arc_points(self.clock_diameter/2, start = math.pi - 0.2, stop = 0.2, origin = pya.DPoint(0, 0))).to_itype(self.layout.dbu))
    ...
    def _build_leads(self):
        ...
        y_offset = self.island_to_island_distance/2 if self.island_to_island_distance > self.clock_diameter else self.clock_diameter/2
        bending_angle = self.bending_angle
        lead_points = [pya.DPoint(self._width_untapered/2, y_offset),
                       pya.DPoint(-self._width_untapered/2, y_offset),
                       pya.DPoint(-self._width_untapered/2, y_offset - self._height_untapered),
                       pya.DPoint(-self._width_tapered/2, y_offset - self._height_untapered - self._height_tapered),
                       
                       pya.DPoint(self._width_tapered/2, y_offset - self._height_untapered - self._height_tapered),
                       pya.DPoint(self._width_untapered/2, y_offset - self._height_untapered)]
        straight_lead_region = pya.Region(pya.DPolygon(lead_points).to_itype(self.layout.dbu))
        bent_lead_polygon = pya.DPolygon([pya.DPoint(-self._width_tapered/2, 0), 
                                                    pya.DPoint(-self._width_tapered/2, - self._length_bent_section),
                                                    pya.DPoint(self._width_tapered/2, - self._length_bent_section), 
                                                    pya.DPoint(self._width_tapered/2,0)
                                                    ])
        angle_patch_polygon = pya.DPolygon([pya.DPoint(-self._width_tapered/2,0), pya.DPoint(-self._width_tapered/2*math.cos(math.radians(bending_angle)), - self._width_tapered/2*math.sin(math.radians(bending_angle))), pya.DPoint(0,0)])
        angle_patch_polygon = pya.DCplxTrans(1,0, False, pya.DPoint(0,y_offset - self._height_untapered - self._height_tapered)) * angle_patch_polygon
        angle_patch_region = pya.Region(angle_patch_polygon.to_itype(self.layout.dbu))
        
        bent_lead_polygon = pya.DCplxTrans(1,bending_angle, False, pya.DPoint(0,y_offset - self._height_untapered - self._height_tapered)) * bent_lead_polygon
    ...
    def _make_ports(self):
        ...
        island_midpoint_height = float(self.island_extent[1])/2 + self.island_to_island_distance/2
        ...
        if self.sim_tool == "q3d":
            ...
            translations = [
                ...
                pya.DPoint(gap_width/2 - port_width, island_midpoint_height- port_height/2),
                pya.DPoint(island_width/2, -island_midpoint_height - port_height/2),
                pya.DPoint(gap_width/2 - port_width, -island_midpoint_height - port_height/2),
            ]
            port_names = ["island1_signal", "island1_ground", "island2_signal", "island2_ground"]
            for i, (trans, name) in enumerate(zip(translations, port_names)):
                ...
                self.add_port(name, trans + pya.DPoint((-1*(i%2) + 1)*port_width, port_height/2), pya.DVector(-2*(i%2) + 1, 0))
        
        elif self.sim_tool == "eig":
            port_polygon = pya.DBox(0, 0, 4, 2)
            # Eigenmode ports
            leads_length = self._height_untapered + self._height_tapered + self._length_bent_section
            translations = [
            ...
            for i, (trans, name) in enumerate(zip(translations, port_names)):
                port_poly = pya.DCplxTrans(1, 0, False, trans)*port_polygon
                port_regions += pya.Region(port_poly.to_itype(self.layout.dbu))
                self.add_port(name, trans + pya.DPoint(2, 2*(i%2)), pya.DVector(0, 2*(i%2) - 1))

        return port_regions
    @classmethod
    def get_sim_ports(cls, simulation):
        return [WaveguideToSimPort("port_0", side="top"),
