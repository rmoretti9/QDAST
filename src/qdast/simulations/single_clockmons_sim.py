...
from qdast.dimensioned_chips.single_clockmons_00 import SingleClockmons00
...
class SingleClockmonsSim(Simulation):

    def build(self):
        chip = self.add_element(
            SingleClockmons00, sim_tool="eig", with_squid=False, n=24
        )
        self.cell.insert(
        ...
        self.ports.extend(
            [
                InternalPort(
                    1,
                    *self.etched_line(
                        refpoints["qb_0_port_island1"], refpoints["qb_0_port_island2"]
                    ),
