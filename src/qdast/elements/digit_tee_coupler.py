...
class DigitTee(SmoothCapacitor):
    ...
    def build(self):
        ...
        tee = self.add_element(WaveguideCoplanarSplitter, lengths=[150, 150, 30])
        tee_trans = pya.DTrans(1, False, refp["port_a"].x - 30, refp["port_a"].y)
        ...
        wg_trans = pya.DTrans(0, False, refp["port_b"].x, refp["port_b"].y)
        ...
        open_to_ground = pya.Region(
            pya.DBox(-self.a / 2 - self.b, 0, self.a / 2 + self.b, 12).to_itype(
                self.layout.dbu
            )
        )
        open_to_ground_trans = [
            pya.DTrans(0, False, refp_tee["port_b"].x, refp["port_b"].y + 150),
            pya.DTrans(0, False, refp_tee["port_a"].x, refp["port_a"].y - 150 - 12),
            pya.DTrans(
                math.pi / 2, False, refp_wg["port_b"].x + 12, refp_wg["port_b"].y
            ),
        ]
