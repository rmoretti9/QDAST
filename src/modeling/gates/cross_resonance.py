...
class CRModel:
    ...
    def zx_strength(self, drive_strength):
        D = self.w1_use - self.w2_use
        a1 = self.a1_use
        self.zx = -self.J * drive_strength / D * (
            a1 / (a1 + D)
        ) + self.J * drive_strength**3 * a1**2 * (
            3 * a1**3 + 11 * a1**2 * D + 15 * a1 * D**2 + 9 * D**3
        ) / 2 / D**3 / (
            a1 + D
        ) ** 3 / (
            a1 + 2 * D
    ...
    def ix_strength(self, drive_strength):
        D = self.w1_use - self.w2_use
        a1 = self.a1_use
        self.ix = -self.J * drive_strength / (
            D + a1
        ) + D * a1 * self.J * drive_strength**3 / (D + a1) ** 3 / (2 * D + a1) / (
            2 * D + 3 * a1
        )
    ...
    def get_transmon_object(self, w, C):
        def tune_frequency(x, C):
            ...
            tmon = scq.Transmon(
                EJ=x, EC=EC, ng=0, ncut=11, truncated_dim=self.truncated_dim
            )
            return abs(tmon.E01() - w)

        EJ = fsolve(tune_frequency, 10, args=(C))[0]
        EC = e_charge**2 / (C) / 2 / planck_h * 1e-9
        tmon_tuned = scq.Transmon(
            EJ=EJ, EC=EC, ng=0, ncut=11, truncated_dim=self.truncated_dim
        )
        return tmon_tuned
    ...
    def zi_strength(self, drive_strength):
        ...
        inner2 = (
            0.5
            * a1
            * (48 * a1**2 / D**3 + 118 * a1 / D**2 + 36 * a1 / (2 * a1 + D) ** 2)
        )
    ...
    def get_coupled_system(self):
        ...
        zpf_correction1 = (
            -1j * (self.tmon1.EJ / 8 / self.tmon1.EC) ** (1 / 4) * 1 / np.sqrt(2)
        )
        zpf_correction2 = (
            -1j * (self.tmon2.EJ / 8 / self.tmon2.EC) ** (1 / 4) * 1 / np.sqrt(2)
        )
    ...
    def diagonalize(self):
        ...
        E_01 = self.hilbertspace.energy_by_bare_index(
            (0, 1, 0), subtract_ground=True
        )  # Qubit 1 in ground state, qubit 2 in excited state (|01⟩)
        E_10 = self.hilbertspace.energy_by_bare_index(
            (1, 0, 0), subtract_ground=True
        )  # Qubit 1 in excited state, qubit 2 in ground state (|10⟩)
        E_11 = self.hilbertspace.energy_by_bare_index(
            (1, 1, 0), subtract_ground=True
        )  # Both qubits in excited state (|11⟩)
    ...
    def time_evolution(
        self, init_state, drive_strength, tlist, m2, echo=False, tsteps=101
    ):
        op_list = []
        ...
        cancel_term = self.cancel_amp * (
            np.cos(phi_cancel) * sx1 + np.sin(phi_cancel) * sy1
        )
        ...
        Hp = (
            ...
            * (
                ...
                + m2
                * drive_strength
                * (np.cos(0.0) * sx2 + np.sin(0.0) * sy2)  # if you want m2 phase too
                + cancel_term
                + rotary_term
            )
        )
            ...
            for t, time in enumerate(tlist):
                if time == 0:
                    ...
                    output1 = qt.mesolve(
                        Hp,
                        init_state,
                        time_subarray[0 : int(len(time_subarray) / 2)],
                        [],
                        [],
                    )
                    psiEcho = (
                        qt.tensor(qt.gates.rx(np.pi), qt.identity(2))
                        * output1.states[-1]
                    )
                    output2 = qt.mesolve(
                        Hm,
                        psiEcho,
                        time_subarray[int(len(time_subarray) / 2) :],
                        [],
                        op_list,
                    )
