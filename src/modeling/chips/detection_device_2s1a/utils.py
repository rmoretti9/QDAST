...
def get_transmission_line_tee_points(filename, tee_a, tee_b, qubit_name=None):
    ...
    transmission_line_tee_points = [
        ...
        + fl_traits[3]
        + fl_traits[4]
        + (tee_a + 2 * tee_b),  # tee to split point
...
class NetworkAnalysis:
    ...
    def t1_purcell_readout(self, qb_frequency, c_sigma, with_purcell_filter=True):
        ...
        cir = rf.Circuit(cnx)
        ntw = cir.network
        freq_span_s1 = np.linspace(
            options["frequency"][0], options["frequency"][1], options["n_points"]
        )
    ...
    def t1_purcell_coupler(self, qb_frequency, options_cplr):
        ...
        ntw = cir.network
        freq_span_s1 = np.linspace(
            options_cplr_copy["frequency"][0],
            options_cplr_copy["frequency"][1],
            options_cplr_copy["n_points"],
        )
        qb_idx = np.argmin(abs(freq_span_s1 - qb_frequency))
        Y_real = ntw.y[qb_idx, 0, 0].real
        t1_purcell_overall = qubit_c / Y_real

        options_cplr_copy["cc"] = [
            1e-22,
            1e-22,
        ]  # almost zero coupling to the bus coupler
        cnx = assemble_network(options_cplr_copy)
        cir = rf.Circuit(cnx)
        ntw = cir.network
        qb_idx = np.argmin(abs(freq_span_s1 - qb_frequency))
        ...
        t1_purcell_coupler_only = (
            1 / t1_purcell_overall - 1 / t1_purcell_readout_only
        ) ** (-1)
