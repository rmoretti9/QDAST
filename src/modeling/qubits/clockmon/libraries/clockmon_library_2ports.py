...
def clockmon_library_2ports(deembed = 200, port_id = "0_2", alternative_geometry = False):
    """
    ...
    if alternative_geometry:
        file_path = os.path.join(dir_name, "sweeps/clockmon_capacitance_library_" + port_id + "_sensing_sim_q3d_results.csv")
    else:
    ...
    cplr2_widths = df['coupler_widths'].apply(lambda x: float(ast.literal_eval(x)[int(port_id.split("_")[1])])).values
...
def clockmon_coupling_libraries(deembed = 200, port_id = "0_2", alternative_geometry = False):
    dir_name = os.path.dirname(os.path.abspath(__file__))
    if alternative_geometry:
        file_path = os.path.join(dir_name, "sweeps/clockmon_capacitance_library_" + port_id + "_sensing_sim_q3d_results.csv")
    else:
...
def clockmon_cqr_to_ground(deembed = 200, port_id = "0_2", ground_id = 0, alternative_geometry = False):
    dir_name = os.path.dirname(os.path.abspath(__file__))
