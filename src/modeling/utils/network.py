"""Utility functions to assemble skrf networks for different quantum device configurations.
"""
...
def assemble_network(options):
    ...
    elif network_type == "two_coupled_t1":
        return two_coupled_qubits_network_t1(options)
    elif network_type == "T1_Purcell_estimation":
        return T1_Purcell_estimation_network(options)
    elif network_type == "T1_Purcell_estimation_simple":
        return T1_Purcell_estimation_network_simple(options)
...
def simple_network(options):
    ...
    freq = rf.Frequency(
        start=f_min, stop=f_max, unit="GHz", npoints=options["n_points"]
    )
    ...
    for i, (ck, cqr, rr_c, rr_l, qb_c, qb_l) in enumerate(
        zip(
            options["ck"],
            options["cqr"],
    ...
    for i in range(n_qubits + 1):
        ...
        cnx.append(
            [(cplr[i], 1), (readout_res_c[i], 0), (readout_res_l[i], 0), (qplr[i], 0)]
        )
...
def tl_resonator_network(options):
    ...
    for i, (ck, ck_gnd, cqr, rr_c, rr_l, qb_c, qb_l) in enumerate(
        zip(
            options["ck"],
    ...
    feedline_capacitor_to_ground = capacitor(
        options["cfl_to_ground"], name="cfl_to_ground"
    )
    ...
    for i in range(n_qubits + 1):
        if i == 0:
            cnx.append(
                [
                    (feedline_capacitor, 1),
                    (tl[i + 1], 0),
                    (feedline_capacitor_to_ground, 0),
                ]
            )
...
def T1_Purcell_estimation_network_simple(options):
    f_min, f_max = options["frequency"]
    freq = rf.Frequency(
        start=f_min, stop=f_max, unit="GHz", npoints=options["n_points"]
    )
    ...
    for i, (ck, ck_gnd, cqr, rr_c, rr_l) in enumerate(
        zip(
            options["ck"],
            options["ck_to_ground"],
            options["cqr"],
...
def two_coupled_qubits_network_t1(options):
    ...
    a, b, ep_r = options["a"], options["b"], options["ep_r"]
    cpw = CPW(frequency=freq, w=a, s=b, ep_r=ep_r, h=options["substrate_thickness"])
    port_qubit = rf.Circuit.Port(freq, "port_qubit", z0=50)
    port1 = rf.Circuit.SeriesImpedance(freq, 50, "port1", z0=50)  # Launcher 1
    port2 = rf.Circuit.SeriesImpedance(freq, 50, "port2", z0=50)  # Launcher 2
    tl = []
    cplr = []
...
def T1_Purcell_estimation_network_coupled_qubits(options):
    ...
    for i, fl in enumerate(options["feedline_traits"]):
        tl.append(cpw.line(d=fl / 1e6, unit="m", name="trans_line_" + str(i)))
    for i, (ck, cqr, rr_c, rr_l) in enumerate(
        zip(
            options["ck"],
            options["cqr"],
            options["readout_resonator_c"],
