...
class ClockmonFluxlineSim(Simulation):
    port_island_1 = Param(pdt.TypeBoolean, "Simulate coupling to island 1", True)
    port_island_2 = Param(pdt.TypeBoolean, "Simulate coupling to island 2", False)
    def build(self):
        ...
        if self.port_island_1:
            self.ports.append(InternalPort(2, *self.etched_line(refpoints["port_island1_signal"], refpoints["port_island1_ground"])))
        if self.port_island_2:
            self.ports.append(InternalPort(2, *self.etched_line(refpoints["port_island2_signal"], refpoints["port_island2_ground"])))
        # self.partition_regions = [
        #     {"name": "squid", "face": "1t1", "vertical_dimensions": 0.0, "region": pya.DBox(280, -10, 306, 7)},
        #     {"name": "refine", "face": "1t1", "vertical_dimensions": 5.0, "region": pya.DBox(260, -20, 320, 20)},
        # ]
# Prepare output directory
dir_path = create_or_empty_tmp_directory(Path(__file__).stem + "_output2")
...
simulations = [
    ...
    # ),
    (
        sim_class(layout, **sim_parameters_1, name="fluxline_decay_1", flux_simulation=False),
        AnsysHfssSolution(max_delta_s=0.005, frequency=4.5, maximum_passes=20, sweep_enabled=False),
    ),
    (
        sim_class(layout, **sim_parameters_2, name="fluxline_decay_2", flux_simulation=False),
        AnsysHfssSolution(max_delta_s=0.005, frequency=4.5, maximum_passes=20, sweep_enabled=False),
    )
]
