...
from kqcircuits.util.export_helper import create_or_empty_tmp_directory, get_active_or_new_layout, \
    open_with_klayout_or_default_application, get_simulation_directory
...
sim_class = get_single_element_sim_class(Clockmon, ignore_ports=['port_drive', 'port_0', 'port_island1_signal', 'port_island2_signal'])
...
sim_parameters = {
    ...
    'tls_layer_thickness': [4.8e-9 * 1e6, 0.3e-9 * 1e6, 2.4e-9 * 1e6],  # MA, MS, and SA,
...
mer_correction_path = get_simulation_directory(f"{sim_parameters['name']}_elmer_metal_edge")
...
for sim_tool in sim_tools:
    ...
    ansys_export_parameters.update({
        ...
        'post_process': PostProcess('produce_epr_table.py',
                                    sheet_approximations={
                                        ...
                                        'MS': {'thickness': 0.3e-9, 'eps_r': 11.4, 'background_eps_r': 11.45},
    ...
    elmer_export_parameters = {
        ...
        'integrate_energies': True,
        'post_process': [PostProcess('produce_q_factor_table.py',
    ...
    sweep_parameters_list = [
        {
            ...
            'partition_regions': [
                        {'name': 'mer',
                         'face': '1t1',
                        'metal_edge_dimensions': [mer_dims_default['gap'], mer_dims_default['metal']],
                        'vertical_dimensions': [mer_dims_default['substrate'], mer_dims_default['vacuum']],
                        },
                        ],
        }]
    ...
    if is_elmer:
        ...
        elmer_export_parameters_default['boundary_conditions'] = {'xmax':{'potential':0}}
        cords_list = []
        for sweep_parameters in sweep_parameters_list:
        ...
        xsection_simulations_metal_edge = create_xsections_from_simulations(
            ...
            vertical_cull=(-mer_dims_default['substrate']*6, mer_dims_default['vacuum']*6),
            mer_box=pya.DBox(metal_edge_region_x/2.-mer_dims_default['metal'],
                             -mer_dims_default['substrate'],
                             metal_edge_region_x/2.+mer_dims_default['gap'],
...
if "elmer" in sim_tools:
    ...
    open_with_klayout_or_default_application(export_simulation_oas(simulations, mer_correction_path, file_prefix="xsection_cut_preview"))
    oas_file_metal_edge = export_simulation_oas(xsection_simulations_metal_edge, dir_path)
